...
bool G[MX][MX], vis[MX], mark[MX];
int n;

void dfs(int v){
	vis[v] = 1;
	rep(i,1,n){
		if(vis[i] == false && G[v][i] && mark[i]){
			dfs(i);
		}
	}
}
...
int main(){
	...
	rep(i,1,it){
		...
		dfs(v[0]);
		//rep(k,0,sz(v)-1) dg3(k, v[k],mark[v[k]]);
		//rep(p,1,n) dg2(p, vis[p]);
		bool flag = 0;
		rep(p,1,n){
			if(mark[p] && (!vis[p])){
				flag = 1;
		...
		if(sz(v)>2){
			...
			rep(p,0,sz(v)-1){
				rep(j,p+1,sz(v)-1){
					rep(k,j+1,sz(v)-1){
						if(G[v[p]][v[j]] && G[v[j]][v[k]] && G[v[p]][v[k]]){
							cnt += 1;
						}
					}
				}
			}
			//rep(k,0,sz(v)-1) dg2(k, v[k]);
			//dg3(cnt, tmp, cnt/tmp);
			if(cnt > mx * tmp){
				mx = cnt/tmp;
				ans = v;
