...
int main(){
	...
	string s;
	cin>>s;
	rep(i, 0, len(s)-1) cnt[s[i]-'a'].pb(i);
	...
	LL prod = 1;
	rer(i, sz(cnt[1]) - 2, 0){
		int ia = UB(cnt[0], cnt[1][i]);
		LL x  = sub(pow2[ia], 1);
		x = mul(x, prod);
		preA[i] = add(preA[i+1], x);
		prod = mul(prod, 2); 
	}
	LL ans = 0;
	LL div = 1;
	rer(i, sz(cnt[1])-2, 0){
		int ic = sz(cnt[2]) - UB(cnt[2], cnt[1][i+1]);
		LL x = sub(pow2[ic], 1);
		LL y = sub(preA[0], preA[i+1]);
		y = mul(y, modexp(div, MOD - 2, MOD));
		div = mul(div, 2);
		ans = add(ans, mul(x, y));
	}
	...
	rer(i,sz(cnt[1])-1, 0){
		int ia = UB(cnt[0], cnt[1][i]);
		int ic = sz(cnt[2]) - UB(cnt[2], cnt[1][i]);
		LL x = sub(pow2[ia], 1);
		LL y = sub(pow2[ic], 1);
		ans = add(ans, mul(x, y));
	}
