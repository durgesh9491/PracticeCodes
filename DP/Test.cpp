...
//LL dp[MX][MX];
LL partition(int sum, int largestNumber, VI &v){
    if (largestNumber==0){
		//if(sz(v)) v.pop_back();
        return 0;
	}
    if (sum==0){
		rep(i,0,sz(v)-1) cout<<v[i]<<" ";
		cout<<endl;
        return 1;
	}
    if (sum<0){
    ...
    //LL &ret = dp[sum][largestNumber];
    //if(ret != -1) return ret;
    v.pb(largestNumber);
    int x = partition(sum-largestNumber,largestNumber, v);
    if(sz(v)) v.pop_back();
    int y = partition(sum,largestNumber-1, v);
    return x + y;
}

int main(){
	while(1){
		//rep(i,0,MX-1) rep(j,0,MX-1) dp[i][j]=-1;
		int sum;
		scanf("%d",&sum);
		VI v;
		printf("%lld\n",partition(sum, sum, v));
