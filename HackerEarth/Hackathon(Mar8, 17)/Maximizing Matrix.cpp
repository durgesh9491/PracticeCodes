...
bool solve(int n, VI &ref, LL sum){
	 if(n < 0) return 1;
	 if(dp.find(mp(n, sum)) != dp.end() || sz(ref) == N/2) {
		 if(sum > mx){
			 mx = sum;
			 fans = ref;
		 }
	 ...
	 dp[mp(n, sum)] = solve(n-1, ref, sum);
	 if(sum + send[n] <= half){
		 ref.pb(send[n]);
		 dp[mp(n, sum + send[n])] = solve(n-1, ref, sum + send[n]);
		 ref.pop_back();
	 }
	 return 1;
}
...
int main(){
	...
	VLL sumA(n, 0), sumB(n, 0);

	rep(i,0,n-1){
		rep(j,0,n-1){
			cin>>A[i][j];
			if(j == 0) sumA.pb(A[i][j]);
			else sumA.pb(sumA[i-1] + A[i][j]);
		}
	}
	rep(i,0,n-1){
		rep(j,0,n-1){
			cin>>B[i][j];
			if(j == 0) sumB.pb(B[i][j]);
			else sumB.pb(sumB[i-1] + B[i][j]);
		}
	}
	VPII res;
	rep(i,0,n-1){
	...
	cout<<sz(res)<<endl;
	rep(i,0,sz(res)-1) cout<<res[i].ff<<" "<<res[i].ss;
	return 0;
}
