...
int main(){
	...
	LL n,f,type,q;
	inp(n);
	rep(i,1,n) inp(A[i]),cnt[A[i]]++;
	rep(i,1,n) freq2[i]= INF;
	int maxi = -1, mini= INF;
	rep(i,1,n){
		int x = cnt[A[i]];
		freq2[x] = min(freq2[x],i);
		maxi = max(maxi, x);
	}
	rer(i,maxi,1){
		if(freq2[i]!=INF){
			freq[i]= min(mini, freq2[i]);
			mini= min(mini, freq2[i]);
		}
		else 
		freq[i]=min(mini,freq[i+1]);
	}
