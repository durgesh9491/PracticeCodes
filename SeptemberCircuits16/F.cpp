...
VI G[MAX];
double dp[MAX][MAX], allowed;
int target, start;

double solve(int current,int prev){                                                         
	if(current == target) return 0;  
	double &ret = dp[current][prev];
	if(ret >= 0) return ret;
	ret = INFLL;
	int sr = current;
	double srx =  point[sr].ff; 
	double sry =  point[sr].ss;
	double prevx =  point[prev].ff; 
	double prevy =  point[prev].ss;
	double srxx = srx - prevx;
	double sryy = sry - prevy;
	rep(i,0,sz(G[sr])-1){
		int des = G[sr][i];
		double desx =  point[des].ff; 
		double desy =  point[des].ss;
		double desxx = desx-srx;
		double desyy = desy-sry;
		double cost = sqrt(dist(srx,sry,desx,desy));
		double num = srxx*desxx + sryy*desyy;
		double den = sqrt(sq(srxx) + sq(sryy)) * sqrt(sq(desxx) + sq(desyy));
		double angle = acos(num/den)*180/PI;
		if(angle<=allowed){
			ret = min(ret, solve(des, sr) + cost);
		}
	}
	return ret;
}
...
int main(){
	...
	int n,m;
	double x,y;
	scanf("%d%d%lf%d%d", &n, &m, &allowed, &start, &target);
	--start,--target;
	rep(i,1,n){
		scanf("%lf%lf", &x, &y);
		point.pb(mp(x,y));
	...
	double ans = INFLL;
	rep(i,0,sz(G[start])-1){
		double cost = sqrt(dist(point[start].ff, point[start].ss, point[G[start][i]].ff, point[G[start][i]].ss)); 
		ans = min(ans, solve(G[start][i], start) + cost);
	}
	if(ans < INFLL)
	printf("%0.3lf",ans);
	else  puts("-1");
