...
#define INFLL                8611686018427387904LL
#define PI                   acos(-1.0)
...
int main(){
	...
	double x1,x2,y1,y2,v1,v2,d1,d2;
	T(){
		...
		rep(i,1,100007){
			mid=(l+r)/2;
			d1= sqrt(sq(mid-x1) + sq(y1));
			d2= sqrt(sq(x2-mid)+ sq(y2));
			val = d1/v1 + d2/v2;
			if(ans==0) ans=val;
			else
		    ans = min(ans, val);
		    if((v2/v1)*sqrt(sq(x2-mid)+sq(y2)) <= ((x2-mid)/(mid-x1))*sqrt(sq(x1-mid)+sq(y1))){
				l=mid+eps;
			}
			else{
				r=mid-eps;
